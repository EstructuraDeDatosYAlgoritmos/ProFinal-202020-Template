...
def rutasCirculares(dataBase, tiempoi, tiempof, estacioni):
    ...
    ltArcos = graph.edges(dataBase['connections'])
    rutas = 0
    ltrutasCirculares = list.newList(datastructure='ARRAY_LIST')
    
    for i in range(1, list.size(ltArcos)+1): 
        estacion  = list.getElement(ltArcos, i)
        if str(estacioni) == estacion['vertexA']:
            estacionf = estacion['vertexB']
            try:
                existe_ar = graph.getEdge(dataBase['connections'], estacionf, estacioni)
